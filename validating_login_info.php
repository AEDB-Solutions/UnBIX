...
function try_login($matricula , $pass, $db_colluns, $erros)
{
	if(validate_login_info($matricula, $pass, $db_colluns))
	{
		...
		echo '<html><head><meta http-equiv="Refresh" content="1;pagina_inicial/index1.php"></head></html>';
		exit;
	}

	else if($erros['passErr'] == ' ' && $erros['matriculaErr'] == ' ')
	$erros['generalErr'] = 'Matricula ou senha Inválida! ';
	//$erros['generalErr'] = 'Matricula ou senha Inválida!'$matricula;
...
function validate_login_info($matricula, $pass, $db_colluns)
{
...
function setting_basic_info($matricula, $pass)
{
	$row = picking_user_datas($matricula, $pass);
...
function picking_user_datas($matricula, $pass)
{
	$find_row = new db_query();
	$find_row->set_find_row("select Matricula,Nome,Email,Userid from Users WHERE Matricula = ? and Senha = ?",array($matricula, hash('sha256',$pass)));
